...
def download_spot(symbol):
    ...
    """
      ...
      :return:
      """
    t1 = Thread(target=get_binance_data, args=(symbol, 'spot', "2017-1-1", "2018-1-1"))
    t2 = Thread(target=get_binance_data, args=(symbol, 'spot', "2018-1-1", "2019-1-1"))
    t3 = Thread(target=get_binance_data, args=(symbol, 'spot', "2019-1-1", "2020-1-1"))
    t4 = Thread(target=get_binance_data, args=(symbol, 'spot', "2020-1-1", "2021-5-20"))
...
def download_future(symbol):
    ...
    t3 = Thread(target=get_binance_data, args=(symbol, 'spot', "2021-4-1", "2020-5-20"))
...
if __name__ == '__main__':
    ...
    # 下载现货的数据
    download_spot("BTCUSDT")

    # 下载合约的数据
    # download_future("BTCUSDT)
